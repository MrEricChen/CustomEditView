package com.mrericchen.customeditview
...
class CustomEditView : EditText {
    ...
    private fun buttonManager(canvas: Canvas) {
        when (mClearButtonMode) {
            ...
            CustomEditView.ClearButtonMode.WHILEEDITING -> drawBitmap(canvas, getRect(hasFocus() && text.isNotEmpty()))
    ...
    private fun getRect(isShow: Boolean): Rect {
        ...
        top = if (isShow) (measuredHeight) / 2  else 0
